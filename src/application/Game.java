  ...
  /*boolean canMove() {
    ...
    for (int x = 0; x < 4; x++) {
      for (int y = 0; y < 4; y++) {
        ...
        if (t.value == 0) {
          // If there is an empty tile, there is a valid move
          return true;
        }

        // Check horizontal and vertical neighbors for valid moves
        if (x > 0 && (t.value + tileAt(x - 1, y).value) % 3 == 0) {
          return true;
        }
        if (y > 0 && (t.value + tileAt(x, y - 1).value) % 3 == 0) {
          return true;
        }
      }
    }
